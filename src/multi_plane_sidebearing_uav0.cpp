...
{
    ros::init(argc, argv, "multi_plane_sidebearing_uav0_Node");
    ros::NodeHandle nh;
    ros::Subscriber status_sub                  = nh.subscribe<mavros_msgs::State>("uav0/mavros/state", 10, status_cb);    
    ros::Subscriber curr_attitude_sub           = nh.subscribe<geometry_msgs::PoseStamped>("uav0/mavros/local_position/pose",100,state_attitude_cb);
    ros::Subscriber speed_sub                   = nh.subscribe<mavros_msgs::VFR_HUD>("uav0/mavros/vfr_hud",100, state_speed_cb);
    ros::Publisher setpoint_attitude_pub        = nh.advertise<mavros_msgs::AttitudeTarget>("uav0/mavros/setpoint_raw/attitude", 100);
    ...
    ros::Subscriber jw_topic_sub = nh.subscribe<geometry_msgs::PoseStamped>("uav0/mavros/jw_topic", 10, PID_TEST_cb);
...
////////////////////////////////////////////////////////////////////
    ...
    while(ros::ok()){
        ...
        yaw_cmd             = 0;
        UAV_pos[0]          = current_attitude.pose.position.x + Init_pos[0];
        UAV_pos[1]          = current_attitude.pose.position.y + Init_pos[1];
        ...
        ROS_INFO("================================");
        double rt = sqrt(pow(UAV_pos[0],2)+pow(UAV_pos[1],2));
        ROS_INFO("radius  : %f", rt);
        ROS_INFO("Yaw  : %f", Current_attitude[2]*180/3.14);        // ROS_INFO("roll  : %f", Current_attitude[0]*180/3.14);
