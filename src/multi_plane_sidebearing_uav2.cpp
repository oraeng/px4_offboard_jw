...
{
    ros::init(argc, argv, "multi_plane_sidebearing_uav2_Node");
    ros::NodeHandle nh;
    ros::Subscriber status_sub                  = nh.subscribe<mavros_msgs::State>("uav2/mavros/state", 10, status_cb);    
    ros::Subscriber curr_attitude_sub           = nh.subscribe<geometry_msgs::PoseStamped>("uav2/mavros/local_position/pose",100,state_attitude_cb);
    ros::Subscriber speed_sub                   = nh.subscribe<mavros_msgs::VFR_HUD>("uav2/mavros/vfr_hud",100, state_speed_cb);
    ros::Publisher setpoint_attitude_pub        = nh.advertise<mavros_msgs::AttitudeTarget>("uav2/mavros/setpoint_raw/attitude", 100);
    ...
    ros::Subscriber jw_topic_sub = nh.subscribe<geometry_msgs::PoseStamped>("uav2/mavros/jw_topic", 10, PID_TEST_cb);
